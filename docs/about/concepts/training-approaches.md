...
With this context on training approaches, the next step is to understand what an environment is and the {ref}`core-components` that make up a training environment in NeMo Gym. When you're ready to get hands-on, see {doc}`/training-tutorials/index` for training tutorials.
